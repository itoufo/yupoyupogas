...
/* ===== ログ出力（7分割：N列〜P列、12星座：R列〜T列、ランキング：H列〜J列） ===== */
function addLog(sheet, stepName, request, response, startTime, endTime) {
  ...
  // 機能ごとにログ列を変更（ランキング：H列（8列目）、12星座：R列（18列目）、7分割：N列（14列目））
  const logColumn = stepName.includes('ランキング') ? 8 : stepName.includes('12星座') ? 18 : 14;
